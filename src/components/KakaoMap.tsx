...
export default function KakaoMap({ lat, lng, placeName }: { lat: number, lng: number, placeName: string }) {
  ...
  if (!KAKAO_API_KEY) {
    return (
      <div className="w-full h-full min-h-[400px] bg-slate-50 flex flex-col items-center justify-center text-slate-400 p-6 text-center border-2 border-dashed border-slate-200 rounded-[2rem]">
        <MapPin size={48} className="text-slate-300 mb-4" />
        <h3 className="font-extrabold text-xl text-slate-500 mb-2">지도 영역</h3>
        ...
        <div className="text-[11px] mt-2 p-4 bg-slate-100 rounded-xl font-mono text-left inline-block">
          <p className="font-bold text-slate-500 mb-1">✓ 조치 방법</p>
    ...
    <>
      ...
      {isLoaded ? (
        ...
        >
          ...
          >
             {/* 마커 위에 표시할 인포윈도우 */}
             <div className="p-2 w-max rounded-lg font-bold text-navy-950 text-sm whitespace-nowrap shadow-sm">
                🏥 {placeName}
             </div>
          </MapMarker>
        </Map>
        ...
        <div className="w-full h-full min-h-[400px] bg-slate-50 flex items-center justify-center rounded-[2rem] animate-pulse">
          <p className="text-slate-400 font-bold text-sm tracking-widest uppercase">Loading Map...</p>
        </div>
      )}
    </>
  );
