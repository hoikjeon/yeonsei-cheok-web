...
export function LiquidSphere() {
  ...
  // Base relative layout positions (25% closer spacing)
  ...
  useFrame((state) => {
    // 1. Reliable Mouse Tracking using native R3F pointer pool
    const targetX = state.pointer.x * (state.viewport.width / 2.5);
    const targetY = state.pointer.y * (state.viewport.height / 2.5);

    // 2. Smooth Position Tracking (Only the Groups track, completely stable)
    if (mainRef.current) {
    ...
    // 3. Central Micro-Jitter (Vibrates the interior refraction without modifying the outer boundary)
    const time = state.clock.elapsedTime;
    
    // High frequency oscillation for IOR and Thickness will make the visible image inside tremble
