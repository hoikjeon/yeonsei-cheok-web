...
function ReactiveChar({ char, originX, originY, fontSize, color, baseOutlineColor }: { char: string, originX: number, originY: number, fontSize: number, color: string, baseOutlineColor: string }) {
  ...
  // Each line gets a 0.15s gap. Base delay of 0.3s for canvas init.
  ...
  useFrame((state) => {
    ...
    if (!isReady.current) {
      ...
      const progress = Math.min(elapsed / 1.2, 1); // 1.2s duration for the rise
      // Smooth ease-out curve
      const ease = 1 - Math.pow(1 - progress, 3);
      
      // Rise from 1.5 units below to origin
    ...
    // --- PHASE 2: NORMAL INTERACTIVE HOVER BEHAVIOR ---
...
function ReactiveLine({ text, y, fontSize, color, baseOutlineColor }: { text: string, y: number, fontSize: number, color: string, baseOutlineColor: string }) {
  ...
  // Calculate relative widths first
  for (let i = 0; i < chars.length; i++) {
    const char = chars[i];
    const width = (charWidthMap[char] || 0.65) * fontSize;
    positions.push(totalWidth + width / 2); // Center point of the current character
    totalWidth += width;
  }
  
  // To perfectly center the word, we offset everything by -half the total string width
  const startX = -totalWidth / 2;

  return (
    <group>
      {chars.map((char, i) => {
        if (char === " ") return null; // No need to mount empty spaces
  ...
  return (
    <div className="w-full h-screen bg-[#121212] overflow-hidden fixed top-0 left-0 z-0 text-white select-none touch-none">
      ...
      >
        ...
        {/* Simple lighting without WebGL depth shadows to prevent cropping and FBO darkness */}
        <directionalLight 
          position={[5, 10, 5]} 
          intensity={1} 
          color="#ffffff" 
        ...
        {/* Synthetic Environment for realistic glass reflections without external HDRI */}
        {/* Colors changed to pure white/grey to remove any pink/blue neon artifacts on the glass */}
