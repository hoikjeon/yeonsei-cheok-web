...
function NoticeBarPreview({ settings }: { settings: HomeNoticeSettings }) {
  ...
  return (
    <div className="overflow-hidden rounded bg-gradient-to-r from-[#17326F] via-[#284AA5] to-[#3C63C4] text-white shadow-[0_22px_60px_-34px_rgba(10,20,40,0.45)]">
    ...
    <>
      ...
      <div className="mx-auto grid w-full max-w-[1480px] grid-cols-1 gap-10 p-10 xl:grid-cols-[minmax(0,1fr)_420px]">
        <form onSubmit={handleSubmit} className="space-y-8">
          {fetchError && (
            <div className="flex items-start gap-3 rounded border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-bold text-amber-800">
          ...
          <section className="rounded border border-slate-200 bg-white p-8 shadow-sm">
            <div className="mb-6 flex flex-col justify-between gap-4 border-b border-slate-100 pb-5 md:flex-row md:items-center">
              ...
              <label className="flex cursor-pointer items-center gap-3 rounded bg-slate-50 px-5 py-4 ring-1 ring-slate-100 transition hover:bg-slate-100">
                <input
                  type="checkbox"
                  name="is_active"
                  checked={isActive}
            ...
            {isLoading ? (
              <div className="rounded bg-slate-50 py-16 text-center text-sm font-black text-ink-muted">
                설정을 불러오는 중입니다...
              </div>
            ) : (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded bg-primary px-4 py-3 text-sm font-black text-white shadow-blue-glow transition hover:bg-primary-dark disabled:cursor-not-allowed disabled:bg-slate-300 disabled:shadow-none"
                  >
                    <Plus size={17} /> 추가
                  </button>
                </div>
                ...
                <div className="space-y-4">
                  {notices.map((notice, index) => (
                    <div key={index} className="rounded border border-slate-200 bg-slate-50/60 p-5">
                      <div className="mb-4 flex items-center justify-between">
                        <span className="rounded-sm bg-white px-3 py-1 text-xs font-black text-primary ring-1 ring-slate-100">
                          공지 {index + 1}
                        </span>
                        <button
                          type="button"
                          ...
                          className="flex h-9 w-9 items-center justify-center rounded-sm text-ink-muted transition hover:bg-white hover:text-red-500 disabled:cursor-not-allowed disabled:opacity-35"
                          aria-label="공지사항 삭제"
                        >
                          <Trash2 size={17} />
                        </button>
                      ...
                      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_280px]">
                        <label className="space-y-2">
                          ...
                          <input
                            ...
                            className="w-full rounded border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-ink outline-none transition focus:border-primary"
                          />
                        </label>
                        <label className="space-y-2">
                          ...
                          <input
                            ...
                            className="w-full rounded border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-ink-sub outline-none transition focus:border-primary"
                          />
                        </label>
                      </div>
                    </div>
          ...
          <div className="flex justify-end">
            <button
              ...
              className="inline-flex min-w-44 items-center justify-center gap-2 rounded bg-navy-950 px-7 py-4 text-[15px] font-black text-white shadow-premium transition hover:-translate-y-0.5 hover:bg-primary disabled:cursor-not-allowed disabled:bg-slate-300 disabled:shadow-none"
            >
              {isSaving ? '저장 중...' : '설정 저장'}
            </button>
          </div>
        ...
        <aside className="space-y-6 xl:sticky xl:top-28 xl:self-start">
          <section className="rounded border border-slate-200 bg-white p-7 shadow-premium">
            <div className="mb-5">
          ...
          <section className="rounded border border-slate-200 bg-white p-7 shadow-sm">
            ...
            <div className="mt-5 space-y-4">
              {[
                ...
                <div key={item} className="flex items-center gap-3">
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-sm bg-slate-100 text-xs font-black text-primary">
                    {index + 1}
                  </span>
