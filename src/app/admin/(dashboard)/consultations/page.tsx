    ...
    <>
      <header className="bg-white border-b border-slate-200 px-10 py-6 sticky top-0 z-[50] shadow-sm flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium mt-0.5">환자들이 남긴 1:1 상담 신청 내역입니다.</p>
        </div>
      </header>
      ...
      <div className="p-10 space-y-8 max-w-[1400px] w-full mx-auto">
        ...
        <div className="space-y-4">
          {isLoading ? (
            ...
            <div className="py-20 text-center font-medium text-slate-400 bg-white rounded-3xl border border-slate-100 shadow-sm">내역이 존재하지 않습니다.</div>
          ) : (
              ...
              <div key={cons.id} className={`bg-white border rounded-[2.5rem] overflow-hidden transition-all shadow-sm ${cons.is_checked ? 'border-slate-100 opacity-80' : 'border-emerald-100 ring-2 ring-emerald-50 shadow-emerald-950/5 shadow-xl'}`}>
                <div className="p-8 flex flex-col md:flex-row gap-6 md:items-center">
                  ...
                  <div className="md:w-60 flex items-center gap-4 border-r border-slate-100 pr-6">
                    <span className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${cons.is_checked ? 'bg-slate-100 text-slate-400' : 'bg-emerald-100 text-emerald-600 animate-pulse'}`}>
                      {cons.is_checked ? <CheckCircle2 size={18} /> : <MessageSquare size={18} />}
                    </span>
                    <div>
                      <p className="text-[14px] text-slate-400 font-bold mb-0.5">{cons.is_checked ? '상담확인' : '신규신청'}</p>
                      <p className="text-xl font-black text-navy-950">{cons.name}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-4">
                     <div className="flex items-center gap-3">
                       <div className="w-9 h-9 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 border border-slate-200 shadow-sm"><Phone size={16} /></div>
                       <div>
                          <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Phone</p>
                          <p className="text-[15px] font-bold text-slate-700">{cons.phone}</p>
                       </div>
                     </div>
                     <div className="flex items-center gap-3">
                       <div className="w-9 h-9 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 border border-slate-200 shadow-sm"><Calendar size={16} /></div>
                       <div>
                          <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Submitted At</p>
                          <p className="text-[15px] font-bold text-slate-700">{new Date(cons.created_at).toLocaleString('ko-KR', { dateStyle: 'medium', timeStyle: 'short' })}</p>
                       </div>
                     </div>
                  </div>
                  ...
                  <div className="md:border-l border-slate-100 md:pl-8 flex items-center gap-3">
                     <button
                       ...
                       className="flex items-center gap-2 px-5 py-3 hover:bg-slate-50 rounded-xl transition-all font-bold text-[14px] text-slate-500"
                     >
                ...
                {expandedId === cons.id && (
                  <div className="bg-slate-50/50 p-8 pt-0 border-t border-slate-100 mt--1">
                     <div className="bg-white p-8 rounded-2xl border border-slate-100 shadow-inner">
                        <p className="text-xs font-black text-emerald-600 uppercase tracking-widest mb-3">Consultation Details</p>
                        <div className="text-[16px] text-navy-950 leading-[1.8] font-medium whitespace-pre-line bg-slate-50 p-6 rounded-xl">
                          {cons.message}
                        </div>
                     </div>
                  </div>
