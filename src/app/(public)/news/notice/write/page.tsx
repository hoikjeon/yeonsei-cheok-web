...
import { Home, ChevronRight, UploadCloud, X, ArrowLeft, Send, Megaphone, Pin } from 'lucide-react';
...
export default function NoticeWritePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append('type', isPinned ? 'notice_pinned' : 'notice');
    ...
    <main className="min-h-screen bg-slate-50 pt-0 md:pt-[96px]">
      <div className="mx-auto max-w-[1000px] px-4 py-10 sm:px-6 sm:py-14 md:py-16">
        <div className="mb-6 flex flex-wrap items-center gap-1.5 text-[12px] font-bold tracking-tight text-ink-muted sm:mb-10 sm:gap-2 sm:text-[14px]">
          ...
          <Link href="/news/notice" className="text-ink-muted">병원소식 / 공지사항</Link>
          <ChevronRight size={14} strokeWidth={2.5} />
          <span className="text-ink">공지사항 등록</span>
        </div>

        <Link href="/news/notice" className="mb-8 inline-flex items-center gap-2 text-h4 tracking-tight text-ink transition-colors hover:text-primary sm:mb-12">
          <ArrowLeft size={28} strokeWidth={2.5} /> 공지사항 작성
        </Link>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm sm:p-6 md:rounded-[2rem] md:p-14">
          <h2 className="mb-7 flex items-center gap-3 break-keep text-h2 tracking-tight text-ink sm:mb-8 sm:gap-4">
            <Megaphone size={34} className="shrink-0 text-primary sm:h-10 sm:w-10" /> 병원 공지 안내 등록
          </h2>

          <form onSubmit={handleSubmit} className="space-y-7 sm:space-y-10">
            <div className="space-y-4">
              <label className="block text-[16px] font-bold text-ink tracking-tight">공지 제목 <span className="text-red-500">*</span></label>
            ...
            <div className="space-y-4">
              <label className="block text-[16px] font-bold text-ink tracking-tight">공지 내용 <span className="text-red-500">*</span></label>
            ...
            <label className="flex cursor-pointer items-start gap-4 rounded-2xl border-2 border-slate-100 bg-slate-50 p-5 transition-all hover:border-primary/30 sm:p-6">
              <input type="checkbox" checked={isPinned} onChange={(e) => setIsPinned(e.target.checked)} className="mt-1 h-5 w-5 accent-primary" />
              <span className="space-y-1">
                <span className="flex items-center gap-2 text-[16px] font-bold tracking-tight text-ink"><Pin size={16} className="text-primary" /> 상단 고정 (중요 공지)</span>
                <span className="block text-[14px] font-medium leading-[1.7] text-ink-muted">체크하면 목록 상단에 &lsquo;공지&rsquo; 배지와 함께 고정 노출됩니다.</span>
              </span>
            </label>

            <div className="space-y-4">
              <label className="block text-[16px] font-bold text-ink tracking-tight">첨부 이미지 (안내 이미지 등)</label>
              <div className="group relative flex flex-col items-center justify-center rounded-2xl border-2 border-dashed bg-slate-50 p-6 text-center transition-all hover:bg-primary/5 sm:p-8 md:rounded-[2rem] md:p-12">
                ...
                <p className="font-bold text-ink text-[18px]">클릭하여 이미지를 업로드하세요</p>
              </div>
              {files.length > 0 && (
            ...
            <div className="pt-8 border-t border-slate-100 flex flex-col md:flex-row gap-4">
              <Link href="/news/notice" className="flex-[1] flex justify-center items-center py-5 rounded-[1.25rem] bg-slate-100 font-bold text-ink-muted">취소</Link>
              <button type="submit" disabled={isSubmitting} className="flex-[2] flex justify-center items-center gap-3 py-5 rounded-[1.25rem] bg-navy-950 hover:bg-primary text-white font-bold text-[19px] transition-all disabled:opacity-50">
                {isSubmitting ? <div className="w-6 h-6 border-4 border-white/30 border-t-white rounded-full animate-spin" /> : <><Send size={20} /> 공지사항 등록하기</>}
              </button>
            </div>
          </form>
        </motion.div>
