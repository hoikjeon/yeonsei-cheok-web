    ...
    <main className="min-h-screen bg-slate-50 pt-[96px]">
      <div className="max-w-[1000px] mx-auto px-6 py-16">
        <div className="flex items-center gap-2 text-[14px] text-slate-500 font-bold mb-10 tracking-tight">
          ...
          <Link href="/news/academic" className="text-slate-400">병원소식 / 학술소식</Link>
          <ChevronRight size={14} strokeWidth={2.5} />
          <span className="text-navy-950">학술 소식 등록</span>
        </div>

        <Link href="/news/academic" className="inline-flex items-center gap-2 text-navy-950 font-black text-[24px] tracking-tight hover:text-primary transition-colors mb-12">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-[2rem] p-10 md:p-14 shadow-sm border border-slate-100">
          ...
          <form onSubmit={handleSubmit} className="space-y-10">
            ...
            <div className="space-y-4">
              <label className="block text-[16px] font-black text-navy-950 tracking-tight">연구 내용 및 상세 소식 <span className="text-red-500">*</span></label>
            ...
            <div className="space-y-4">
              <label className="block text-[16px] font-black text-navy-950 tracking-tight">관련 이미지 (논문 표지, 학회 사진 등)</label>
              <div className="border-2 border-dashed rounded-[2rem] p-12 flex flex-col items-center justify-center bg-slate-50 hover:bg-primary/5 transition-all group relative">
                ...
                <p className="font-bold text-navy-950 text-[18px]">클릭하여 이미지를 추가하세요</p>
              </div>
              {files.length > 0 && (
            ...
            <div className="pt-8 border-t border-slate-100 flex flex-col md:flex-row gap-4">
              <Link href="/news/academic" className="flex-[1] flex justify-center items-center py-5 rounded-[1.25rem] bg-slate-100 font-black text-slate-500">취소</Link>
