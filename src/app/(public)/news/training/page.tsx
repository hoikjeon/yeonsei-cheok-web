    ...
    <main className="min-h-screen bg-slate-50 pt-[96px]">
      <section className="bg-slate-50 border-b border-slate-200">
        <div className="max-w-[1440px] mx-auto px-10 py-16">
          <div className="flex items-center gap-2 text-[14px] text-slate-500 font-bold tracking-tight mb-10">
            ...
            <ChevronRight size={14} strokeWidth={2.5} />
            <span className="text-navy-950">국제 척추내시경 트레이닝 센터</span>
          </div>
          <h1 className="text-[48px] md:text-[56px] font-black text-navy-950 tracking-tighter leading-tight">국제 척추내시경<br />트레이닝 센터</h1>
        </div>
      </section>
      ...
      <section className="bg-white">
        <div className="max-w-[1440px] mx-auto px-10 py-24 border-x border-slate-50 min-h-[800px]">
          <div className="mb-20">
            <h2 className="text-[36px] font-black leading-[1.3] text-navy-950 tracking-tighter mb-4">세계로 뻗어나가는 연세척의 의료 기술</h2>
            <p className="text-slate-500 text-[18px] font-bold tracking-tight">국내외 의료진을 대상으로 한 척추 내시경 교육 및 교류 소식입니다.</p>
          </div>
          ...
          <div className="space-y-8">
            ...
            {newsCount > 0 ? (
                ...
                {news?.map((item: any) => (
                  <Link href={`/news/training/${item.id}`} key={item.id} className="group flex flex-col bg-white rounded-[2rem] overflow-hidden border border-slate-100 hover:border-primary/20 hover:shadow-2xl transition-all duration-500">
                    <div className="aspect-[4/3] bg-slate-100 relative overflow-hidden">
                      ...
                      <div className="absolute top-6 left-6 px-4 py-1.5 bg-blue-600/90 backdrop-blur-md rounded-full text-white text-[11px] font-black uppercase tracking-widest flex items-center gap-1.5"><GraduationCap size={12} /> Training Center</div>
                    </div>
                    <div className="p-8 flex flex-col flex-1">
                      <h3 className="text-[22px] font-black text-navy-950 leading-[1.4] tracking-tighter group-hover:text-primary transition-colors line-clamp-3 mb-6">{item.title}</h3>
              ...
              <div className="py-32 text-center text-slate-300 font-bold tracking-tight">등록된 트레이닝 센터 소식이 없습니다.</div>
            )}

            <div className="flex justify-center pt-10 border-t border-slate-100">
              <Link href="/news/training/write" className="inline-flex items-center gap-3 px-12 py-5 bg-navy-950 text-white rounded-[1.25rem] hover:bg-primary font-black shadow-lg shadow-navy-950/20 transition-all text-[18px]">
                <PenSquare size={20} strokeWidth={2.5} /> 트레이닝 소식 등록하기
              </Link>
            </div>
          </div>
        </div>
