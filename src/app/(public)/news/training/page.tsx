        ...
        <div className="mx-auto min-h-0 max-w-[1440px] border-x border-slate-50 px-4 py-14 sm:px-6 sm:py-16 md:min-h-[800px] lg:px-10 lg:py-24">
          <div className="space-y-6 sm:space-y-8">
                ...
                {news?.map((item: HospitalNews) => (
                  <Link href={`/news/training/${item.id}`} key={item.id} className="group flex flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white transition-all duration-500 hover:border-primary/20 hover:shadow-2xl md:rounded-[2rem]">
                    <div className="aspect-[4/3] bg-slate-100 relative overflow-hidden">
                      ...
                      <div className="absolute left-4 top-4 flex items-center gap-1.5 rounded-full bg-blue-600/90 px-3 py-1.5 font-montserrat text-[10px] font-black uppercase tracking-widest text-white backdrop-blur-md sm:left-6 sm:top-6 sm:px-4 sm:text-[11px]"><GraduationCap size={12} /> Training Center</div>
                    </div>
                    <div className="flex flex-1 flex-col p-5 sm:p-6 md:p-8">
                      <h3 className="mb-5 line-clamp-3 break-keep text-[19px] font-black leading-[1.45] tracking-tight text-ink transition-colors group-hover:text-primary sm:text-[20px] md:mb-6 md:text-[22px]">{item.title}</h3>
            ...
            <div className="flex justify-center pt-10 border-t border-slate-100">
              <Link href="/news/training/write" className="inline-flex w-full items-center justify-center gap-3 rounded-xl bg-navy-950 px-5 py-4 text-[16px] font-black text-white shadow-lg shadow-navy-950/20 transition-all hover:bg-primary sm:w-auto sm:rounded-[1.25rem] sm:px-12 sm:py-5 sm:text-[18px]">
