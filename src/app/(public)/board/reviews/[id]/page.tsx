    ...
    <main className="min-h-screen bg-white pt-[96px]">
      ...
      <section className="bg-slate-50 border-b border-slate-100">
        <div className="max-w-[1200px] mx-auto px-6 py-6 flex items-center gap-2 text-[13px] text-slate-400 font-bold tracking-tight">
          ...
          <ChevronRight size={12} strokeWidth={3} />
          <span className="text-navy-950 truncate max-w-[200px]">{review.title}</span>
        </div>
      </section>
      ...
      <div className="max-w-[1000px] mx-auto px-6 py-16 md:py-24">
        ...
        <div className="mb-12 border-b border-slate-100 pb-12">
          ...
          <h1 className="text-[32px] md:text-[44px] font-black text-navy-950 tracking-tighter leading-[1.2] mb-8">
            {review.title}
          </h1>
          ...
          <div className="flex flex-wrap items-center gap-6 text-[15px] font-bold text-slate-400 tracking-tight">
            ...
            <div className="text-slate-400">조회수 124</div>
          </div>
        </div>
            ...
            {review.image_urls.map((url: string, index: number) => (
              ...
              >
                ...
                <div className="px-6 py-4 bg-white/80 backdrop-blur-md border-t border-slate-50 flex justify-between items-center">
                  <span className="text-[13px] font-bold text-slate-400 italic">Photo {index + 1}</span>
                  <span className="text-[12px] font-black text-primary/40 tracking-widest uppercase">Yonsei Cheok</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="prose prose-slate max-w-none mb-24">
          <p className="text-[18px] md:text-[20px] font-medium text-slate-700 leading-[1.9] tracking-tight whitespace-pre-wrap">
            {review.content}
          </p>
        </div>
        ...
        <div className="bg-slate-50 border border-slate-100 rounded-[1.5rem] p-8 mb-20 text-center">
          <p className="text-[15px] font-bold text-slate-500 tracking-tight">
            <strong className="text-navy-950 font-black">※ 안내사항</strong><br className="md:hidden"/>
        ...
        <div className="flex justify-center items-center gap-4">
          <Link 
            href="/board/reviews"
            className="flex items-center gap-2 px-10 py-5 bg-slate-100 hover:bg-slate-200 text-slate-600 font-black rounded-[1.25rem] transition-all active:scale-95 text-[17px]"
          >
