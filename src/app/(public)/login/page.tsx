...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (params.get('reason') === 'consultation-private') {
      setMessage({
        type: 'error',
        text: '상담 내용은 작성자 본인만 확인할 수 있습니다. 로그인 후 이용해 주세요.',
      });
    } else if (params.get('reason') === 'members-only') {
      setMessage({
        type: 'error',
